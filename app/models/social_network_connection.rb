...
class SocialNetworkConnection < ActiveRecord::Base
  ...
  def autocomplete?
    ...
    return value
      ...
      # ==== Examples
      #
      def self.get_networks(includeall = true,groupothers=true)
        ...
        if(networkslist.nil?)
          ...
          networkslist.map do |social_network|
            if(NETWORKS.keys.include?(social_network.network))
              known_network = true
      ...
      def self.get_edit_networks(includeother = true)
        return_networks = []
        NETWORKS.keys.sort.each do |network|
      ...
      def self.get_name(network)
        NETWORKS.keys.include?(network) ? SocialNetwork::NETWORKS[network][:displayname] : "Other Network (#{network.upcase})"
      end
      
      def self.get_network_url(network,accountid)
        network_url = '' # default
        if(NETWORKS.keys.include?(network))
          urlformat = NETWORKS[network][:urlformat]
          if(!urlformat.nil?)
      ...
      def self.has_editable_url?(network)
        if(!NETWORKS.keys.include?(network))
          return true
        elsif
