...
## constants
  ...
  def self.periodic_activity_by_person_id(options = {})
    ...
    maxdate = self.maximum(:activity_at).to_date
    if(maxdate < end_date)
      end_date = maxdate
    end
    ...
    persons.each do |person_id|
      ...
      returndata[person_id]['dates'] = base_scope.pluck('DATE(activity_at)').uniq
      returndata[person_id]['days'] = returndata[person_id]['dates'].size
      returndata[person_id]['items'] = base_scope.count('DISTINCT(question_id)')
