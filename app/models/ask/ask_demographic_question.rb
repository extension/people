...
class AskDemographicQuestion < ActiveRecord::Base
  include CacheTools
  ...
  serialize :responses
  has_many :demographics, class_name: 'AskDemographic', foreign_key: 'demographic_question_id'
  ...
  def self.mean_response_rate
    ...
    response_rate[:responses] = (AskDemographic.where("user_id in (#{limit_pool.join(',')})").where("demographic_question_id IN (#{limit_list.join(',')})").count / limit_list.size)
    response_rate
  end
