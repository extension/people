...
## filters
## associations
  has_many :evaluation_answers, class_name: 'AskEvaluationAnswer', foreign_key: 'evaluation_question_id'
...
## scopes
  ...
  def self.mean_response_rate
    ...
    response_rate[:responses] = (AskEvaluationAnswer.where("question_id IN (#{limit_pool.join(',')})").where("evaluation_question_id IN (#{limit_list.join(',')})").count / limit_list.size)
    response_rate
  end
