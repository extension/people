...
## associations
  has_many :question_events, :class_name => 'AskQuestionEvent', foreign_key: 'question_id'

  belongs_to :assignee, :class_name => "AskUser", :foreign_key => "assignee_id"
  belongs_to :current_resolver, :class_name => "AskUser", :foreign_key => "current_resolver_id"
  belongs_to :location, class_name: 'AskLocation'
  belongs_to :county, class_name: 'AskCounty'
  belongs_to :original_location, :class_name => "AskLocation", :foreign_key => "original_location_id"
  belongs_to :original_county, :class_name => "AskCounty", :foreign_key => "original_county_id"
  belongs_to :submitter, :class_name => "AskUser", :foreign_key => "submitter_id"
  belongs_to :assigned_group, :class_name => "AskGroup", :foreign_key => "assigned_group_id"
  belongs_to :original_group, :class_name => "AskGroup", :foreign_key => "original_group_id"
...
## scopes
  ...
  def detected_location
    AskLocation.find_by_geoip(self.user_ip)
  end

  def detected_county
    AskCounty.find_by_geoip(self.user_ip)
  end
  ...
  def tags
    AskTag.includes(:taggings).where('taggings.taggable_type = ?','Question').where('taggings.taggable_id = ?', self.id)
  end
  ...
  def self.evaluation_data_csv(filename)
    CSV.open(filename,'wb') do |csv|
      ...
      eval_columns = []
      AskEvaluationQuestion.order(:id).active.each do |aeq|
      ...
      response_questions = AskEvaluationAnswer.pluck(:question_id).uniq
